...
function fizzbuzzPlusPlus(nums, words) {
  ...
  // calculate products of given nums
  const product = nums.reduce((acc, num) => acc * num, 1);
  
  // iterate from 1 to product of given nums
  for (let i = 1; i <= product; i++){
    // initialize variable to concatenate words
    let str = '';
    for (let j = 0; j < nums.length; j++){
      // if current number is a multiple, concat corresponding words
      if (i % nums[j] === 0){
        str += words[j]
      }
    }
    // if no number was added use current num
    result.push(str || i)
  }
